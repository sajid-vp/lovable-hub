          ...
          {items.map((item, index) => {
            ...
            return (
              <TileWrapper
                key={item.id}
                {...(tileProps as any)}
                className="group relative flex items-center gap-2 sm:gap-2.5 rounded-xl sm:rounded-2xl px-2.5 sm:px-3 py-2 bg-card border border-border/50 active:scale-[0.97] shadow-sm transition-transform duration-150 animate-scale-in tap-highlight app-touch"
              ...
              >
                ...
                <span className="font-semibold text-xs sm:text-sm truncate relative z-10 flex-1">
                  {item.title}
                </span>
                ...
                {isExternal && (
                  <ExternalLink className="h-3 w-3 sm:h-3.5 sm:w-3.5 flex-shrink-0 text-muted-foreground/50" />
                )}
              </TileWrapper>
            );
          })}
