...
export function EmployeeSpotlight({ variant = "default" }: EmployeeSpotlightProps) {
  ...
  if (variant === "compact") {
    return (
      <section className="h-full flex flex-col">
        ...
        <div className="relative p-4 rounded-2xl bg-card border border-border shadow-[0_4px_20px_-4px_rgba(0,0,0,0.12)] flex-1">
          {/* Decorative gradient */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 h-full">
            {spotlightEmployees.map((spotlight, index) => {
              ...
              return (
                <div
                  key={spotlight.id}
                  className="relative rounded-xl bg-muted border border-border p-3 transition-all duration-300 hover:bg-card hover:border-[hsl(var(--turquoise))]/40 hover:shadow-lg hover:shadow-[hsl(var(--turquoise))]/5 hover:-translate-y-0.5 group flex flex-col"
                >
  ...
  return (
    <section className="space-y-3">
      ...
      {/* Spotlight Card */}
      <div className="relative overflow-hidden rounded-2xl bg-card border border-border shadow-[0_4px_20px_-4px_rgba(0,0,0,0.12)] p-6 group hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.15)] transition-all duration-300">
        {/* Decorative gradient */}
